...
#import "WPContextMenuCell.h"

@interface WPContextMenuCell () <UIGestureRecognizerDelegate>
...
@implementation WPContextMenuCell
...
- (void)setUp
{
    ...
    self.editable = YES;
    self.moreEditable = YES;
    self.deleteEditable = YES;
    self.moreOptionsButtonTitle = @"More";
    self.deleteButtonTitle = @"";
...
- (void)layoutSubviews
{
    ...
    [self.deleteButton setImageEdgeInsets:UIEdgeInsetsMake((self.deleteButton.frame.size.height - 38)/2, (self.deleteButton.frame.size.width - 36)/2, (self.deleteButton.frame.size.height - 38)/2, (self.deleteButton.frame.size.width - 36)/2)];
}
...
{
     if (self.moreEditable && self.editable) {
...
{
    if (self.deleteEditable && self.editable) {
        if (!_deleteButton) {
            CGRect frame = CGRectMake(0., 0., 100., CGRectGetHeight(self.actualContentView.frame));
